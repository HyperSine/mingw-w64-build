...
function pkg_download() {
    ...
    local PKG_SRC_URL="https://ftp.gnu.org/gnu/ncurses/${PKG_SRC_FILENAME}"
    local PKG_SIG_URL="https://ftp.gnu.org/gnu/ncurses/${PKG_SIG_FILENAME}"
...
function pkg_extract() {
    ...
    if [[ ${SCRIPT_OPTION_FORCE_UPDATE} == "yes" || ! -d ${PKG_SOURCE_PATH} ]]; then
        ...
        func_enter_directory ${PKG_SOURCE_PATH}
            patch -p1 -i ${PKG_PATCH_PATH}/work_around_changed_output_of_GNU_cpp_5.x.patch
    ...
    if [[ ${SCRIPT_OPTION_FORCE_UPDATE} == "yes" || ! -x ${PKG_CONFIGURE_PATH}/config.status ]]; then
        ...
        func_enter_directory ${PKG_CONFIGURE_PATH}
            ${PKG_SOURCE_PATH}/configure \
                ...
                --enable-pc-files \
                --with-pkg-config \
                --with-pkg-config-libdir=${PKG_PREFIX_PATH}/lib/pkgconfig \
                --disable-rpath \
                --enable-colorfgbg \
                --disable-symlinks \
                --enable-warnings \
