...
function pkg_download() {
    ...
    local PKG_SRC_URL="https://ftp.gnu.org/pub/gnu/libiconv/${PKG_SRC_FILENAME}"
    local PKG_SIG_URL="https://ftp.gnu.org/pub/gnu/libiconv/${PKG_SIG_FILENAME}"
...
function pkg_extract() {
    ...
    if [[ ! -d ${PKG_SOURCE_PATH} ]]; then
        ...
        func_enter_directory ${PKG_SOURCE_PATH}
            func_apply_patch -p1 ${PKG_PATCH_PATH}/0001-compile-relocatable-in-gnulib.mingw.patch
            func_apply_patch -p1 ${PKG_PATCH_PATH}/0002-fix-cr-for-awk-in-configure.all.patch
            func_apply_patch -p1 ${PKG_PATCH_PATH}/fix-pointer-buf.patch
